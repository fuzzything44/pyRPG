...
from objects.tutorial.war_tut import directed_attack
...
class war_enemy(General.enemy_base.enemy_base):
    ...
    def collide(this, obj):
        if obj.type == "player": # If collides with a player, damage them. Wait to damage again for 500ms
            if obj.attributes["name"] + "-attack-del" not in this.attributes["effects"]:
